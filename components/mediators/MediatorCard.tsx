 ...
 * ✅ حُذف LoveCoin وعرض الرصيد تماماً
 * ✅ onSubscribe → onRequestMediation
 */

import { useState, useEffect, useRef } from 'react';
import { motion }                       from 'framer-motion';
import { MapPin, ChevronLeft, MessageCircle, ShieldCheck } from 'lucide-react';
...
interface Props {
  ...
  onRequestMediation:  (m: MediatorRow) => void;   // ← بديل onSubscribe
  onOpenDetail:        (m: MediatorRow) => void;
  onMessage?:          (m: MediatorRow) => void;
}

export function MediatorCard({
  mediator, rank, isAuthenticated, onRequestMediation, onOpenDetail, onMessage,
}: Props) {

  const handleWhatsApp = () => {
    if (!mediator.whatsapp) return;
    window.open(`https://wa.me/${mediator.whatsapp.replace(/\D/g, '')}`, '_blank');
  };

  const handleTikTok = () => {
    if (!mediator.tiktok) return;
    window.open(`https://www.tiktok.com/@${mediator.tiktok.replace('@', '')}`, '_blank');
  };
    ...
    >
          ...
          <motion.div whileHover={{ scale: 1.04 }}
            ...
            {mediator.avatar_url
              ...
              : <div className="w-full h-full flex items-center justify-center icon-wrap"
                  ...
                  <Icon i={ShieldCheck} size={26} color="var(--text-tertiary)" />
                </div>}
          </motion.div>
      ...
      <div className="flex gap-2 mt-4">
        ...
        <div className="flex-1 rounded-2xl px-2 py-2 text-center"
          ...
          <p style={{ fontSize: 'var(--text-2xs)', color: 'rgba(212,175,55,0.6)', fontWeight: 700 }}>إجمالي</p>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-2">
        {mediator.isSubscribed ? (
          <motion.div initial={{ scale: 0.96 }} animate={{ scale: 1 }}
            ...
            <Icon i={ShieldCheck} size={16} color="#D4AF37" /> طلبك قيد المعالجة ✓
          </motion.div>
        ) : (
          <motion.button whileTap={{ scale: 0.97 }}
            onClick={() => isAuthenticated && onRequestMediation(mediator)}
            ...
            <Icon i={ShieldCheck} size={16} color="#fff" /> طلب وساطة
          </motion.button>
        )}
