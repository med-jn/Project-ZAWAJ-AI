      ...
      >
        ...
        <div style={{ position: 'absolute', insetInlineStart: 0, insetInlineEnd: 0, bottom: 'calc(var(--nav-h) + 5.5rem)', padding: '0 var(--sp-5)', direction: 'rtl', pointerEvents: 'none' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', flexWrap: 'wrap' }}>
            {!!u.age && (
              <span style={{ color: 'var(--text-main)', fontWeight: 700, fontSize: 'var(--text-md)', textShadow: '0 1px 8px rgba(0,0,0,0.4)' }}>
                {u.age} سنة
              </span>
            )}
            {u.city && (
              <span style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-1)', color: 'var(--text-secondary)', fontSize: 'var(--text-sm)', textShadow: '0 1px 8px rgba(0,0,0,0.4)' }}>
...
function Btn3D({ variant, size, active, busy, onClick, icon }: {
  variant: 'like' | 'pass'; size: number; active: boolean;
  busy?: boolean; onClick: () => void; icon: React.ReactNode;
}) {
  ...
  const boxShadow  = active
    ? `0 2px 0 ${depthColor}, 0 4px 18px ${glowColor}, inset 0 2px 4px rgba(0,0,0,0.35)`
    : `0 6px 0 ${depthColor}, 0 10px 28px ${glowColor}, inset 0 1px 0 rgba(255,255,255,0.18), inset 0 -2px 0 rgba(0,0,0,0.22)`;

  return (
    <motion.button onClick={onClick} disabled={busy}
      whileTap={{ scale: 0.84, y: 4 }} whileHover={{ scale: 1.06, y: -2 }}
      transition={{ type: 'spring', stiffness: 500, damping: 22 }}
      style={{ width: size, height: size, borderRadius: '50%', border: 'none', outline: 'none', cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? 0.35 : 1, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', background: faceColor, boxShadow, transition: 'box-shadow 0.18s, background 0.18s' }}>
      <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'radial-gradient(ellipse at 38% 22%, rgba(255,255,255,0.22) 0%, transparent 62%)', pointerEvents: 'none' }} />
      {icon}
    </motion.button>
  );
}
