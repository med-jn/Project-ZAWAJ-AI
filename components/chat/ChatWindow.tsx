'use client';
/**
 * components/chat/ChatWindow.tsx — ZAWAJ AI v3.1
 * ✅ zIndex رُفع إلى 1100 ليظهر فوق PageHeader (zIndex: 1000)
 */
...
interface MessageRowProps {
  msg: ChatMessage; isMine: boolean; isPressed: boolean;
  onSwipe: () => void; onPressStart: () => void;
  onPressEnd: () => void; onDelete: () => void;
}

function MessageRow({ msg, isMine, isPressed, onSwipe, onPressStart, onPressEnd, onDelete }: MessageRowProps) {
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative' }}
      ...
      <div dir="rtl" style={{ display: 'flex', flexDirection: 'column', alignItems: isMine ? 'flex-start' : 'flex-end', gap: 2 }}>
        <div style={{
          ...
          {msg.reply_to_content && (
            <QuoteBubble content={msg.reply_to_content} type={msg.reply_to_type ?? 'text'} isMine={isMine} />
          )}
          {msg.message_type === 'voice' && msg.audio_url ? (
            ...
            <p style={{ margin: 0, fontSize: 'var(--chat-msg-size, var(--text-sm))', lineHeight: 1.6, color: isMine ? '#ffffff' : 'var(--text-main)', wordBreak: 'break-word' }}>
              {msg.content}
            </p>
          )}
          <AnimatePresence>
            {isPressed && isMine && (
              <motion.button
                ...
                style={{ position:'absolute', top:-12, right:-10, width:28, height:28, borderRadius:'50%', background:'#f87171', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <AnimatePresence>
          {isPressed && (
            <motion.div initial={{ opacity:0, y:-4 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-4 }}
              ...
              {isMine && (msg.is_read ? <CheckCheck size={12} color="#4fc3f7" /> : <Check size={12} color="var(--text-tertiary)" />)}
              <span style={{ fontSize:10, color:'var(--text-tertiary)' }}>{msgTime(msg.created_at)}</span>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
...
export default function ChatWindow({ conversationId, currentUserId, recipient, onBack, onOpenProfile, onBlock }: Props) {
  const { messages, loading, convStatus, recipientTyping, sendMessage, sendVoiceMessage, setTyping, deleteMessage, markConversationRead, acceptConversation } = useChat(conversationId, currentUserId, recipient.id);
  ...
  const [replyTo,      setReplyTo]      = useState<{ id: string; content: string; type: string; } | null>(null);
  ...
  useEffect(() => { if (messages.length > 0) markConversationRead(); }, [messages.length]);
  useEffect(() => { scrollRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages, recipientTyping]);
  ...
  const handleSwipeReply = (msg: ChatMessage) => { setReplyTo({ id: msg.id, content: msg.content || '', type: msg.message_type }); inputRef.current?.focus(); };
  const handlePressStart = (msgId: string) => { longPressTimer.current = setTimeout(() => setPressedId(msgId), 500); };
  const handlePressEnd   = () => { if (longPressTimer.current) { clearTimeout(longPressTimer.current); longPressTimer.current = null; } };

  const handleBlock = async () => {
    setShowMenu(false);
    await supabase.from('blocks').upsert({ blocker_id: currentUserId, blocked_id: recipient.id }, { onConflict: 'blocker_id,blocked_id', ignoreDuplicates: true });
    setIsBlocked(true); onBlock?.(); onBack();
  };

  const showAcceptBanner = !convStatus.is_unlocked && !convStatus.is_free && !convStatus.pending_unlock && messages.some(m => m.sender_id === recipient.id);
  ...
  return (
    <div
      style={{
        ...
        zIndex: 1100, // ✅ أعلى من PageHeader (1000)
      }}
      onClick={() => pressedId && setPressedId(null)}
    >
      {/* HEADER */}
      <header dir="rtl" style={{
        position: 'fixed', top: 0, right: 0, left: 0, zIndex: 1101,
        ...
        <button onClick={() => onOpenProfile?.(recipient.id)} style={{
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <AnimatePresence mode="wait">
              {recipientTyping && (
                <motion.span key="typing" initial={{ opacity:0, y:4 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:4 }}
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <button onClick={() => setShowMenu(v => !v)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-tertiary)' }}>
          ...
          <AnimatePresence>
            {showMenu && (
              <>
                ...
                <motion.div initial={{ opacity:0, scale:0.88, y:-8 }} animate={{ opacity:1, scale:1, y:0 }} exit={{ opacity:0, scale:0.88, y:-8 }} transition={{ duration:0.15 }}
                  dir="rtl" style={{ position: 'absolute', top: 46, left: 0, zIndex: 20, background: 'var(--bg-elevated)', border: '1px solid var(--glass-border)', borderRadius: 16, overflow: 'hidden', width: 150, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
                  {[
                    ...
                    <button key={item.label} onClick={item.action} style={{ width: '100%', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: 'none', cursor: 'pointer', borderBottom: i===0 ? '1px solid var(--glass-border)' : 'none', color: item.color, fontFamily: 'inherit', fontSize: 13, fontWeight: 600 }}>
        ...
        <button onClick={onBack} style={{ background: 'transparent', border: 'none', cursor: 'pointer', width: 44, height: 44, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-main)' }}>
          <ArrowLeft size={22} />
        </button>
      </header>
      ...
      <AnimatePresence>
        {showAcceptBanner && (
          <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} exit={{ opacity:0, height:0 }}
            dir="rtl" style={{ background:'rgba(164,22,26,0.10)', borderBottom:'1px solid var(--border-soft)', padding:'10px 14px', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, overflow:'hidden' }}>
            ...
            <button onClick={acceptConversation} style={{ padding:'5px 14px', borderRadius:20, background:'var(--color-accent)', border:'none', color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer', flexShrink:0 }}>قبول</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showWaitBanner && (
          <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} exit={{ opacity:0, height:0 }}
            dir="rtl" style={{ background:'rgba(234,179,8,0.08)', borderBottom:'1px solid rgba(234,179,8,0.2)', padding:'8px 14px', flexShrink:0, display:'flex', alignItems:'center', gap:7, overflow:'hidden' }}>
            <Clock size={13} color="#ca8a04" />
            <span style={{ fontSize:11, color:'#ca8a04' }}>في انتظار رد {isFemale ? 'الطرف الأخرى' : 'الطرف الآخر'}</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'12px 12px 8px', display:'flex', flexDirection:'column', gap:6 }}>
        {loading ? (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', flex:1, color:'var(--text-tertiary)', fontSize:13 }}>جارٍ التحميل...</div>
        ) : (
          <>
            {messages.map(msg => (
              <MessageRow key={msg.id} msg={msg} isMine={msg.sender_id === currentUserId} isPressed={pressedId === msg.id}
                onSwipe={() => handleSwipeReply(msg)} onPressStart={() => handlePressStart(msg.id)}
                onPressEnd={handlePressEnd} onDelete={() => { deleteMessage(msg.id); setPressedId(null); }} />
            ))}
            <AnimatePresence>
              {recipientTyping && (
                <motion.div key="typing-bubble" initial={{ opacity:0, y:8, scale:0.95 }} animate={{ opacity:1, y:0, scale:1 }} exit={{ opacity:0, y:8, scale:0.95 }} transition={{ duration:0.2 }}>
                  <TypingBubble />
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <AnimatePresence>
        {replyTo && (
          <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} exit={{ opacity:0, height:0 }}
            dir="rtl" style={{ background:'var(--bg-elevated)', borderTop:'1px solid var(--glass-border)', padding:'8px 12px', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
            <Reply size={14} color="var(--color-accent)" style={{ flexShrink:0 }} />
            <div style={{ flex:1, minWidth:0 }}>
              <p style={{ margin:0, fontSize:'var(--chat-quote-size, var(--text-2xs))', color:'var(--text-secondary)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                {replyTo.type === 'voice' ? '🎤 رسالة صوتية' : replyTo.content}
              </p>
            </div>
            <button onClick={() => setReplyTo(null)} style={{ background:'transparent', border:'none', cursor:'pointer', padding:4, flexShrink:0, color:'var(--text-tertiary)', display:'flex', alignItems:'center' }}>
              <X size={14} />
            </button>
          </motion.div>
        )}
      ...
      <div dir="rtl" style={{ padding:'8px 12px', paddingBottom:'max(var(--safe-bottom, env(safe-area-inset-bottom, 0px)), 8px)', background:'var(--bg-surface)', borderTop:'1px solid var(--glass-border)', flexShrink:0 }}>
        {isBlocked ? (
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, background:'var(--glass-bg)', border:'1px solid var(--glass-border)', borderRadius:30, padding:'4px' }}>
            <motion.button whileTap={{ scale:0.82 }} onClick={handleSend} style={{ width:40, height:40, borderRadius:'50%', flexShrink:0, background: hasText ? 'var(--color-accent)' : 'rgba(255,255,255,0.06)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'background 0.2s' }}>
              <Send size={15} color={hasText ? '#ffffff' : 'var(--text-tertiary)'} />
            </motion.button>
            <input ref={inputRef} type="text" dir="rtl" placeholder="اكتب رسالتك..." value={inputText}
              onChange={e => handleChange(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()}
      ...
      <ReportSheet open={showReport} onClose={() => setShowReport(false)} reportedUserId={recipient.id} targetType="conversation" targetId={conversationId} />
    </div>
  );
}